...
if(isset($_POST['change'])){
    ...
    $result = $mysqli->query("SELECT * FROM tblaccounts WHERE email = '$email' AND token = '$token' AND status != 'deactivated'") or die($mysqli->error());
    
    if(mysqli_num_rows($result) > 0){
       $mysqli->query("UPDATE tblaccounts SET password='$newPass', token='', tokenExpiry='0000-00-00 00:00:00' WHERE email = '$email' AND token = '$token' AND status != 'deactivated'") or die($mysqli->error());
       header("location: ../login/");
    }
    else{
        echo "Hacker to";
